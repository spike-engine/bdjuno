...
func proposalCmd(parseConfig *parsecmdtypes.Config) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			mintModule := mint.NewModule(sources.MintSource, sources.StakingSource, parseCtx.EncodingConfig.Marshaler, db)
			slashingModule := slashing.NewModule(sources.SlashingSource, parseCtx.EncodingConfig.Marshaler, db)
			stakingModule := staking.NewModule(sources.StakingSource, slashingModule, sources.LiquidStakingSource, parseCtx.EncodingConfig.Marshaler, db)
